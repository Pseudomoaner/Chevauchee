...
nr_of_cores = 8; % integer, 1 means single core, greater than 1 means parallel
if nr_of_cores > 1
	try
...
%list each root folder containing the timepoints in this array
roots = ["D:\Sean\SurfaceColonyPIV\Mar1_WT-C1TA_OD01"];
for j = 1:size(roots,2);

rootFolder = roots(j);

%do some idiotic shit to get the folder names as strings to use in the loop
d = dir(rootFolder);
folders = d([d(:).isdir]) ;
...
for i = 1:size(folders,2);
    directory = char(strjoin([rootFolder filesep char(folders(i))],""));
...
%directory= 'D:\Sean\SurfaceColonyPIV\Feb2_WT-C1TA_OD01\median_blur\LB-f20_WT-R-C1TA-Y_OD01_1h' ; %directory containing the images you want to analyze
...
%(image1,image2,
%interrogationarea, step, subpixfinder, mask_inpt, roi_inpt,passes,int2,int3,int4,imdeform,repeat,mask_auto,do_pad,do_correlation_matrices,repeat_last_pass,delta_diff_min)
...
s{10,1}='Window deformation';       s{10,2}='*spline'; % '*spline' is more accurate, but slower. NB to match CorrQuality settings from saved analysis settings see line 5500 of PIVlab_GUI.m
...
%secret setting '0' is slid in here. 'do_correlation_matrices'
...
if nr_of_cores > 1
	...
	%disp("parallel pools in operation")
	parfor i=1:size(slicedfilename1,2)  % index must increment by 1
		%disp("inside parallel for loop")
...
% These are actuall velocity based filtering settings
%there should be some kind of choice whether to filter by image or speed
%and which to do first
...
r{3,1}= 'Valid velocities [u_min; u_max; v_min; v_max]';    r{3,2}=[-10; 10; -10; 10];  % Maximum allowed velocities, for uncalibrated data: maximum displacement in pixels
r{4,1}= 'Stdev check?';                                     r{4,2}=1;                   % 1 = enable global standard deviation test
r{5,1}= 'Stdev threshold';                                  r{5,2}=8;                   % Threshold for the stdev test
...
if nr_of_cores >1 % parallel
	...
	parfor PIVresult=1:size(x,1)
		...
		[u_filt{PIVresult,1}, v_filt{PIVresult,1},typevector_filt{PIVresult,1}]= ...
			post_proc_wrapper(u{PIVresult,1},v{PIVresult,1},typevector{PIVresult,1},r,do_interpolate,user_corr_thresh,correlation_map{PIVresult,1});
...
save(fullfile(directory, [extractAfter(directory,rootFolder) '_frames_result.mat']));

end
%end the loop over subfolders
...
function [u_filt, v_filt,typevector_filt] = post_proc_wrapper(u,v,typevector,post_proc_setting,paint_nan,thresh,corr_map)
...
%this seems like a good place to stick the corelation filter
[u_filt,v_filt] = PIVlab_correlation_filter(u_filt,v_filt,thresh,corr_map);
...
%the most useful aspect of image based filtering is the correlation
%coefficient threshold, which is implemented by PIVlab_correlation_filter (u,v,corr_filter_thresh,resultslist{12,frame})
%what is resultslist {12, frame}. It is the corelation value at that
%location. where are these held in the commandline batch processing loop?
%resultsList{12, } is the corelation map
%function [u_image_filt,v_image_filt] = PIVlab_image_filter (do_contrast,do_bright,x,y,u,v,contrast_filter_thresh,bright_filter_thresh,A,B,rawimageA,rawimageB)
%...?
%end
