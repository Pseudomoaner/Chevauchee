%Code to generate IBM simulations with mixed motility types and combat
%types
clear all
close all
...
RootSim = '/home/omeacock/Documents/SPRruns/Driveby/FightyFightyRuns';
outputSprMatName = 'SprResults_f1_%f_f2_%f_fire1_%f_fire2_%f.mat';
...
patchSettings.seedFrac = 0.5; % Fraction of seeds that should be associated with population 2
patchSettings.seedDensity = 0.001;
...
for f1Ind = 1:size(fs,2)
    for f2Ind = f1Ind:size(fs,2)
        for fireInd = 1:size(fires,2)
            f1 = fs(f1Ind);
            f2 = fs(f2Ind);
            fire1 = fires(fireInd);
            fire2 = fires(end);
            ...
            intermediateField.fireCells = ones(size(intermediateField.xCells))*fire1;
            intermediateField.fCells = ones(size(intermediateField.xCells))*f1;
            patchSettings.fireRate = fire2;
            patchSettings.force = f2;
            ...
            fullSprMatOut = fullfile(RootSim,sprintf(outputSprMatName,f1,f2,fire1,fire2));
